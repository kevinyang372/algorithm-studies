...
class TrafficLight:
    def __init__(self):
        self.traffic_lights = [True, True]
        self.lock = Semaphore(1)

    def carArrived(self, car_id, direction, turnGreen, crossCar):
        light_id = (direction - 1) // 2

        self.lock.acquire()
        if not self.traffic_lights[light_id]:
            turnGreen(light_id)
        
        self.traffic_lights[1 - light_id] = False
        self.traffic_lights[light_id] = True
        
        crossCar(car_id, light_id, direction)
        self.lock.release()


def turnGreen(light_id):
    print(f"Traffic Light On Road {light_id} Is Green")
    time.sleep(0.5)


def cross(car_id, light_id, direction):
    print(f"Car {car_id} Has Passed Road {light_id} In Direction {direction}")
    time.sleep(2)
...
if __name__ == "__main__":
    ...
    for ind in range(len(cars)):
        Thread(target=traffic_light.carArrived, args=(cars[ind], directions[ind], turnGreen, cross)).start()
        time.sleep(1)
