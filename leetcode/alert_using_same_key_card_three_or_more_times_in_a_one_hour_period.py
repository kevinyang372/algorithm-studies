...
def alertNames(self, keyName: List[str], keyTime: List[str]) -> List[str]:
    ...
    def convert_string_to_number(date):
    ...
    for name, time in zip(keyName, keyTime):
        itime = convert_string_to_number(time)
        stack[name].append(itime)
        
    for key in stack:
        stack[key].sort()
        for i in range(2, len(stack[key])):
            if stack[key][i] - stack[key][i - 2] <= 60:
                to_report.add(key)
                break
    
    return sorted(list(to_report))