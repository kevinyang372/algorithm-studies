...
def findBalanced(students, num):
    
    students.sort()
    j = num - 1
    
    diff = sum([students[j] - students[n] for n in range(j)])
    res = diff
    
    for dj in range(j + 1, len(students)):
        diff -= students[dj - 1] - students[dj - num]
        diff += (students[dj] - students[dj - 1]) * (num - 1)
        res = min(diff, res)
...
for t in range(T):
    ...
    print("Case #%s: %s" % (t + 1, findBalanced(temp, P)))